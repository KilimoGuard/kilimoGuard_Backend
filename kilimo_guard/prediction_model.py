...
class PredictionModel:   
    ...
    mock_dataset_path = os.path.join(settings.MEDIA_ROOT, "datasets", "MOCK_WEATHER_DATA.csv")
    mock_model_path = os.path.join(settings.MEDIA_ROOT, "models", "mock_weather_model.pkl")         

    # Functions Section
    def train_model():
        print("-"*30)
        print("🛠 Importing dataset ... ")        
        df = pd.read_csv(PredictionModel.mock_dataset_path)                
        ...
        with open(PredictionModel.mock_model_path, 'wb') as file:
    ...
    def make_prediction(user_input):        
        with open(PredictionModel.mock_model_path, 'rb') as file:
            mock_model = pickle.load(file)
        ...
        user_input_features = mock_model.named_steps['preprocessor'].transform(user_input_df)

        # Prediction for User Input
        user_probabilities = mock_model.named_steps['classifier'].predict_proba(user_input_features)
